...
This was not an anomaly. In the 12 months between April 2025 and April 2026, authzed.com documented **14 distinct MCP security incidents**: WhatsApp message exfiltration; a GitHub MCP prompt injection exfiltrating private repository data including financial information; an Asana MCP logic flaw exposing enterprise customer data across tenant boundaries (~1,000 enterprise customers notified, 35 days exposure); a Smithery supply-chain breach exposing Docker and Fly.io credentials for 3,000+ apps; MCP Inspector (Anthropic's own debugging tool) — unauthenticated RCE exposing filesystem and API keys; an Oura MCP malware campaign delivering StealC credential harvesters to live users (February 2026); an nginx-ui MCP authentication bypass granting complete nginx service control across 2,600+ live infrastructure instances (March 2026); a Gemini MCP tool 0-day (January 2026, unauthenticated RCE on Google's own implementation); and a core STDIO architectural flaw affecting 150M+ downloads that Anthropic declined to patch — OX Security executed commands on six live production platforms including LiteLLM, LangChain, and IBM LangFlow to demonstrate the scope. The attack surface runs from AI tool descriptions to infrastructure management.
...
Multiple MCP scanners exist. Several have autofix. The scanner market is occupied. AgentPreflight is not 10x because it detects MCP/skill issues first — multiple scanners do that now. It is 10x because it makes the secure path faster and more reliable: from poisoned agent artifact to fixed, rescan-proven PR in under two minutes, without executing the server. No existing tool — not mcp-scan, not Snyk Agent Scan, not AgentAuditKit — provides the complete loop: offline-first static scan → AI-generated Codex patch proposal → developer review → rescan proof. The wedge is not detection breadth or even fix capability — it is **the full loop**.
...
- The MCP specification explicitly requires clients to treat tool annotations from untrusted servers as untrusted — but provides no enforcement mechanism. Anthropic declined to modify the STDIO architecture (OX Security, April 2026). GitHub Issue #630 — "MCP Server terminology creates dangerous user misconceptions" — was closed as "not planned." The spec warns; the protocol won't enforce; the ecosystem can't self-clean. The gate must exist outside the protocol layer.
...
- **Codex is structural, not decorative.** The scanner was designed Codex-first — every rule produces a Codex-ready remediation context from day one. The `--codex` flag is a live API call, not a template fill. The demo shows it. The rescan proves it held. The 30-test suite includes mocked Codex API integration tests that verify SYSTEM_PROMPT constraints hold across rule families and Rule 5 blocks re-injection of prompt-injection payloads. Codex is in the remediation path, the demo path, and the test path — not bolted on after the fact.
