...
That was not an edge case. In 12 months (April 2025–April 2026): 14 documented MCP incidents, including GitHub MCP prompt injection exfiltrating private repo data with financial information, Asana MCP cross-tenant leak exposing ~1,000 enterprise customers for 35 days, Smithery supply chain breach hitting 3,000+ apps, Oura MCP malware campaign delivering StealC credential harvesters (Feb 2026), nginx-ui MCP auth bypass giving attackers complete nginx service control across 2,600+ live infrastructure instances (Mar 2026), and a Gemini MCP 0-day at Google. Equixly's 2025 audit found 45% of vendors dismissed security findings as "theoretical." They were wrong — eleven months later came StealC. In April 2026, Anthropic declined to patch the STDIO architectural flaw (150M+ downloads). The MCP spec requires clients to treat untrusted tool annotations as untrusted — but provides no enforcement. The gate must be built outside the protocol layer.
