...
- **First-mover advantage on the Codex-patch loop for MCP security.** OWASP published MCP and Agentic Skills security guidance in 2025; multiple scanners now map to that taxonomy. None close the loop with automated Codex diff-patches and a rescan that confirms trust_score=100. No existing tool — not mcp-scan, not Snyk Agent Scan, not AgentAuditKit — delivers the complete workflow: offline-first scan → AI-generated reviewer-ready patch proposal → rescan proof. The security standard exists. The attack evidence is live. The remediation tooling is being built now — AgentPreflight is first on the complete scan-to-patch-to-proof loop.
