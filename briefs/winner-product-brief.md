...
**Codex in the test path:** The 30-test suite includes mocked Codex API integration tests that verify the SYSTEM_PROMPT constraints hold across rule families — confirming Rule 5 blocks patch proposals from re-introducing prompt-injection patterns and that the 5-line redacted snippet format never leaks secrets or file paths. Codex is active in the remediation path, the demo path, and the test path.
